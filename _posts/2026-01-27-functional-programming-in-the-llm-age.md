...
As interesting as that direction is, it isn't the only avenue worth investigating. Another approach to improving type-checking is through the adoption of [effect systems](https://en.wikipedia.org/wiki/Effect_system). The basic idea behind effect-systems is to offer a way to separate certain aspects of **what** a program is doing from **how** the program is doing it. For example, Scala's [Cats-Effect](https://typelevel.org/cats-effect/) library allows one to write code without having to worry about the concurrency model the code will run under. This matters because concurrency (along with I/O concerns, and dealing with exceptions) is one of the main causes of code being non-deterministic (and therefore impure). Effect-systems make it easier to keep core program logic pure, and therefore easier for type-checkers to automatically verify as correct.

Even if LLMs didn't exist, enhanced purity would be a desirable goal in its own right, by enabling the elimination of entire classes of program error, just as static typing and bounds-checking eliminated entire classes of errors which used to be ubiquitous. However, this In a world in which publicly-available code is increasingly auto-generated by LLMs, statically-verifiable purity becomes crucial, as it opens the door to an approach to synthetically generate high-quality training data. 

Through a reinforcement-learning technique called [self-play](https://en.wikipedia.org/wiki/Self-play), in which agents are able to play against themselves, it has been possible to train them to [superhuman levels](https://en.wikipedia.org/wiki/AlphaZero) of performance in games like Go/Baduk and Chess. Given such a prior record of success, it is easy to see why this technique [appeals](https://arxiv.org/abs/2502.00212) [strongly](https://neurips.cc/virtual/2025/loc/san-diego/poster/116121) to groups trying to leverage Lean for automatic theorem-proving. Thanks to the Curry-Howard correspondence, we know that theorem proving and program writing are essentially the same thing, so if self-play can be made to work in the Lean context, it's reasonable to expect it to work with other functional programming languages which have more widespread adoption (e.g. Haskell and Scala).

One can imagine a time in the near future when, thanks to the adoption of this approach, the frontier language models are able to consistently churn out Haskell or Scala code of a far higher quality than they can even for statically-typed languages as popular as Java or C#, let alone for weakly or barely typed languages like JavaScript, for which the quality of publicly-available code also suffers from their popularity with audiences unfamiliar with good programming practices. If such a scenario came to pass, it would also address the biggest reason why Haskell and Scala never really took off in the first place, namely the inability of most programmers to grasp how to work with the strange new concepts these languages seemed to demand of them. If all one is doing is writing specifications for an agent, and one can be sure that the agent will correctly, verifiably produce code meeting those specifications, then the need to understand the generated code disappears, just as hardly any programmers today need to understand the machine-code instructions their high-level languages compile (or are interpreted) to. "Programming" then becomes a matter of correctly understanding specifications, and correctly transcribing them.

Of course, if the above scenario **does** come to pass, the fact that Haskell or Scala (or even Lean) is the underlying language used for code generation will also probably cease to be relevant for the most part. So, in a sense, there really wouldn't be much of a functional-programming "revival" to speak of, or at least, not one visible to most programmers. But I imagine that in this brave new world where LLMs routinely spit out code of a rigour and performance most human practitioners could never aspire to, the fate of most programming languages will be to fade into obscurity anyway. 
