...
- the ability to **subitise** - a technical term denoting our ability to recognise how many there are of a small number of objects without having to explicitly count them. Most people have the ability to do this for 1 to 3 objects, and apparently so do many other primates.
...
Amongst the many issues with Roman numerals are
 - The ambiguity in how digits are represented - while `I, II, III` (1, 2, 3), `IV, V, VI, VII, VIII` (4, 5, 6, 7, 8) and `IX, X, XI` (9, 10, 11) seem distinct enough in isolation, imagine trying to parse such representations when they are used in the context of several other digits (e.g. ` DCCLXXXIX`).
 - The inconsistency in how values are built up. With `I, II, III` the notion is **additive**, but then with `IV, V` it become **subtractive**. It then returns to additivity with `VI, VII, VIII` before becoming subtractive again with `IX, X`. This back-and-forth is then repeated for values like `XL` (40), `XC` (90), `CD` (400) and `CM` (900). One might expect the pattern to continue with values like 4,000 and 9,000, but evidently this is not the case.
 - The lack of any symbols to represent the concept of zero. It was not until the Middle Ages that scholars began to use the term "Nulla" to signify the absence of something.
